...
// userAgents is the list of random User-Agent strings used when
// constructing WebSocket transport headers for proxy nodes.
...
// RandUserAgent returns a random User-Agent string from the built-in list.
func RandUserAgent() string {
	return userAgents[rand.Intn(len(userAgents))]
}

// knownCiphers is the set of recognized Shadowsocks cipher methods.
...
// VerifyMethod checks whether the cipher method is a recognized
// Shadowsocks cipher. It returns nil for valid ciphers.
func VerifyMethod(cipher, password string) error {
	if strings.ToLower(cipher) == "none" || strings.ToLower(cipher) == "plain" {
		return nil
	}
	if password == "" {
		return nil
	}
	if !knownCiphers[strings.ToLower(cipher)] {
		return fmt.Errorf("unknown cipher method: %s", cipher)
	}
	return nil
}
